...
def R2(S): # TODO: Is there a better transitive closer algorithm or similar to use?
    """
    Input:
    ...
    for p, qs in S.items():
            ...
            to_add = set()   # All r's such that pRqRr for some q
            for q in qs:
...
def get_supported_combinations(ab, cd, supp_plus):
    """
    Input:
        ab: A tuple (a,b)
        cd: A tuple (c,d)
...
def get_equiv_r_plus(R_plus):
    ...
    equiv_rel = {p: {p} for p in R_plus.keys()}   # R_plus is reflexive, so (p,p) is in R_plus for all p in the extended support.
...
def get_order_r_plus(Q_set, R_plus): 
    """
    Input:
...
def get_canoncial_dag(order_R_plus):
    """
    Input:
